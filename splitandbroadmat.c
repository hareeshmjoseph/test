/****************************************************************************
    * FILE: mpi_broadcast.c
    * Matrix Addition Program using MPI. Split Matrix A & Broadcast Matrix B
...
MPI_Isend(&mat_b[low_bound][0], (upper_bound - low_bound) * NUM_COLUMNS_B,MPI_DOUBLE, i, MASTER_TO_SLAVE_TAG + 2, MPI_COMM_WORLD, &request);
            }
...
MPI_Recv(&mat_b[low_bound][0], (upper_bound - low_bound) * NUM_COLUMNS_B, MPI_DOUBLE, 0, MASTER_TO_SLAVE_TAG + 2, MPI_COMM_WORLD, &status);
        ...
        if (rank == 0) {
            for (i = 1; i < size; i++) {// until all slaves have handed back the processed data
